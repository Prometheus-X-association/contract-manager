...
describe('Billing rules injection and removal test cases for contract', () => {
  ...
  it('Should inject rule-billing-1', async () => {
    ...
    const offering = response.body.contract.serviceOfferings.find(
      (o: any) =>
        ...
        o.participant === data.participant,
    );
    expect(offering).to.exist;
    expect(offering.policies).to.be.an('array');
    const policy = offering.policies.find(
      (p: any) => p.permission[0].target === data.policies[0].values.target,
    );
  ...
  it('Should inject rule-billing-4', async () => {
    ...
    expect(policy.permission[0].duty[0].constraint[0].rightOperand).to.equal(
      data.policies[0].values.currentDateTime,
    );
  ...
  it('Should remove billing policies for a service offering', async () => {
    ...
    expect(entry.policies.length).to.equal(0);
  });
